...
function letterCombinations(digits: string): string[] {
  ...
  const result: string[] = [];
  const numberDigits: number[] = digits.split('').map((e: string) => Number(e));

  const dfs = (arr: number[], comb: string) => {
    if (arr.length === 0) {
      if (comb.length >= 1) {
        result.push(comb);
      }
    ...
    const currNumber: number | undefined = arr.shift();
