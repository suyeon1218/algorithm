...
function combinationSum(candidates: number[], target: number): number[][] {
  const result: number[][] = [];
  const numMap = new Map<string, boolean>();

  const backtracking = (comb: number[], currSum: number) => {
    if (currSum >= target) {
      if (currSum === target) {
        const stringKey = comb.sort((a: number, b: number) => a - b).join(',');
...
function combinationSum2 (candidates: number[], target: number): number[][] {
  ...
  const go = (position: number, combination: number[], sumValue: number) => {
    if (sumValue === target) {
